...
from ood.ood_graph_editor import Graph_Editer,Graph_Editer_Mask
...
def run_fix_mask(args, seed, rewind_weight_mask):
    ...
    gl = Graph_Editer_Mask(args['K'], dataset_tr.graph['num_nodes'], device,adj)
    ...
    optimizer_aug = torch.optim.AdamW(gl.parameters(), lr=args['lr_a'])
    acc_test = 0.0
    best_avg_acc=0
    bset_acc_tests=[]
    ...
    for epoch in range(args['epochs'] ):
        ...
        for m in range(args['T']):
            ori_tensor = net_gcn(features, adj, gragh_editor=True)
            graph_loss = 0
            Loss, Log_p = [], 0
            for k in range(args['K']):
                edge_index_k = gl(adj, dataset_tr.graph['num_nodes'], args['num_sample'], k, args['rate'])
                # print('adj',adj.sum())
                # print('edge_index_k-adj',(edge_index_k-adj).sum())
                output = net_gcn(features, edge_index_k)
                graph_tensor = net_gcn(features, edge_index_k, gragh_editor=True)
                labels = labels.squeeze(dim=-1)
                loss = loss_func(output, labels)
                Loss.append(loss.view(-1))
                # print('ori_tensor',ori_tensor[1,:])
                # print('graph_tensor',graph_tensor[1,:])
                graph_loss = graph_loss + l2_loss(ori_tensor, graph_tensor)
                # print('graph_loss',graph_loss)
                # graph_loss.append(l2_loss(ori_tensor,graph_tensor).view(-1))
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    return best_val_acc['val_acc'], best_val_acc['test_acc'], best_val_acc['epoch'],bset_acc_tests, adj_spar, wei_spar
...
def parser_loader():
    ...
    parser.add_argument('--exp_name', type=str, default='gcn_our_ood_gragheditor')
...
if __name__ == "__main__":
    ...
    for p in range(20):
        ...
        best_acc_val, final_acc_test, final_epoch_list, best_acc_tests, adj_spar, wei_spar = run_fix_mask(args, seed, rewind_weight)
