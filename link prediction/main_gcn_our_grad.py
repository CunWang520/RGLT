...
def run_get_mask(args, imp_num, rewind_weight_mask, dataset_dict):
    ...
    wei_01_grad = torch.zeros_like(model.gcn.net_layer[0].weight_mask_train).cpu()
    wei_02_grad = torch.zeros_like(model.gcn.net_layer[1].weight_mask_train).cpu()
    ...
    for epoch in range(1, args.mask_epoch + 1):
        ...
        wei_01_grad = wei_01_grad + model.gcn.net_layer[0].weight_mask_train.grad.clone().cpu()/ args.mask_epoch
        wei_02_grad = wei_02_grad + model.gcn.net_layer[1].weight_mask_train.grad.clone().cpu()/ args.mask_epoch
        optimiser.step()
        ...
        with torch.no_grad():
            ...
            if acc_val > best_val_acc['val_acc']:
                ...
                best_val_acc['val_acc'] = acc_val
                best_val_acc['epoch'] = epoch
                rewind_weight, adj_spar, wei_spar = pruning_gcn.get_final_mask_epoch02(model, rewind_weight, args,adj_grad,wei_01_grad,wei_02_grad)

        print(
            "IMP[{}] (GCN {} Get Mask) Epoch:[{}/{}], Loss:[{:.4f}] Val:[{:.2f}] Test:[{:.2f}] | Best Val:[{:.2f}] Test:[{:.2f}] at Epoch:[{}] | Adj:[{:.2f}%] Wei:[{:.2f}%]"
            .format(imp_num,
                    args.dataset,
                    epoch,
                    args.mask_epoch,
...
def parser_loader():
    ...
    parser.add_argument('--exp_name', type=str, default='gcn_our', help='')
