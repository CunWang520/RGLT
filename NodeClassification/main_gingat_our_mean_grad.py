...
def run_fix_mask(args, imp_num, rewind_weight_mask):
    ...
    print(
        ...
        .format(args['dataset'],
                ...
                adj_spar,
                wei_spar))
    return best_val_acc['val_acc'] * 100,best_val_acc['test_acc'] * 100,best_val_acc['epoch'],adj_spar,wei_spar
...
def run_get_mask(args, imp_num, rewind_weight_mask=None):
    ...
    wei_01_grad = torch.zeros_like(net_gcn.ginlayers[0].apply_func.mlp.linear.weight_mask_train).cpu()
    wei_02_grad = torch.zeros_like(net_gcn.ginlayers[1].apply_func.mlp.linear.weight_mask_train).cpu()
    ...
    for epoch in range(args['mask_epoch']):
        ...
        adj_grad = adj_grad + net_gcn.adj_mask1_train.grad.clone().cpu()/args['mask_epoch']
        wei_01_grad = wei_01_grad + net_gcn.ginlayers[0].apply_func.mlp.linear.weight_mask_train.grad.clone().cpu()/args['mask_epoch']
        wei_02_grad = wei_02_grad + net_gcn.ginlayers[1].apply_func.mlp.linear.weight_mask_train.grad.clone().cpu()/args['mask_epoch']
        ...
        with torch.no_grad():
            ...
            if acc_val > best_val_acc['val_acc']:
                ...
                if args['net'] == 'gin':
                    rewind_weight, adj_spar, wei_spar = pruning_gin.get_final_mask_epoch02(net_gcn,adj_grad,wei_01_grad,wei_02_grad, rewind_weight, args)
                else:
                    rewind_weight, adj_spar, wei_spar = pruning_gat.get_final_mask_epoch02(net_gcn,adj_grad,wei_01_grad,wei_02_grad, rewind_weight, args)
...
if __name__ == "__main__":
    ...
    print(args)
    # wandb.init(project="gnn_prune", name=args['net'] + '_our_' + args['dataset'])
    ...
    for imp in range(1, 21):
        ...
        best_val_acc,final_test_acc,epoch,adj_spar,wei_spar = run_fix_mask(args, imp, rewind_weight)
