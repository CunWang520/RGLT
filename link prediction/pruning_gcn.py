...
def get_final_mask_epoch(model, rewind_weight, args):
    ...
    rewind_weight['gcn.net_layer.0.weight_mask_train'] = get_each_mask(model.gcn.net_layer[0].state_dict()['weight_mask_train'], wei_thre)
    rewind_weight['gcn.net_layer.0.weight_mask_fixed'] = rewind_weight['gcn.net_layer.0.weight_mask_train']
    rewind_weight['gcn.net_layer.1.weight_mask_train'] = get_each_mask(model.gcn.net_layer[1].state_dict()['weight_mask_train'], wei_thre)
    rewind_weight['gcn.net_layer.1.weight_mask_fixed'] = rewind_weight['gcn.net_layer.1.weight_mask_train']

    adj_spar = rewind_weight['gcn.adj_mask2_fixed'].sum() * 100 / model.gcn.adj_nonzero
    wei_nonzero = rewind_weight['gcn.net_layer.0.weight_mask_fixed'].sum() + rewind_weight['gcn.net_layer.1.weight_mask_fixed'].sum()
    wei_all = rewind_weight['gcn.net_layer.0.weight_mask_fixed'].numel() + rewind_weight['gcn.net_layer.1.weight_mask_fixed'].numel()
    wei_spar = wei_nonzero * 100 / wei_all

    return rewind_weight, adj_spar, wei_spar


def get_final_mask_epoch02(model, rewind_weight, args,adj_grad,wei_01_grad,wei_02_grad):
    adj_percent = args.pruning_percent_adj
    wei_percent = args.pruning_percent_wei
    adj_mask, wei_mask = get_mask_distribution(model.gcn, if_numpy=False)
    ...
    rewind_weight['gcn.adj_mask1_train'] = get_each_mask02(ori_adj_mask, adj_thre,adj_grad,args.pruning_percent_adj,True)
    rewind_weight['gcn.adj_mask2_fixed'] = rewind_weight['gcn.adj_mask1_train']

    rewind_weight['gcn.net_layer.0.weight_mask_train'] = get_each_mask02(
        model.gcn.net_layer[0].state_dict()['weight_mask_train'], wei_thre,wei_01_grad,args.pruning_percent_wei,False)
    rewind_weight['gcn.net_layer.0.weight_mask_fixed'] = rewind_weight['gcn.net_layer.0.weight_mask_train']
    rewind_weight['gcn.net_layer.1.weight_mask_train'] = get_each_mask02(
        model.gcn.net_layer[1].state_dict()['weight_mask_train'], wei_thre,wei_02_grad,args.pruning_percent_wei,False)
    rewind_weight['gcn.net_layer.1.weight_mask_fixed'] = rewind_weight['gcn.net_layer.1.weight_mask_train']

    adj_spar = rewind_weight['gcn.adj_mask2_fixed'].sum() * 100 / model.gcn.adj_nonzero
    wei_nonzero = rewind_weight['gcn.net_layer.0.weight_mask_fixed'].sum() + rewind_weight[
        'gcn.net_layer.1.weight_mask_fixed'].sum()
    wei_all = rewind_weight['gcn.net_layer.0.weight_mask_fixed'].numel() + rewind_weight[
        'gcn.net_layer.1.weight_mask_fixed'].numel()
    wei_spar = wei_nonzero * 100 / wei_all

    return rewind_weight, adj_spar, wei_spar


def get_each_mask02(mask_weight_tensor, threshold,grad,percent,adj=False):
    ones = torch.ones_like(mask_weight_tensor)
    zeros = torch.zeros_like(mask_weight_tensor)
    mask = torch.where(mask_weight_tensor.abs() > threshold, ones, zeros)
    total_zero = mask[mask!=1].shape[0]
    total_non_zero = mask[mask!=0].shape[0]
    print('total_zero:',total_zero)
    reactivate_num = int(0.001*total_non_zero)#0.005
    print('reactivate_num:',reactivate_num)
    ...
    reverse_mask = torch.where(mask_weight_tensor.abs() <= threshold, ones, zeros)
    grad = grad.to(mask_weight_tensor.device)
    # print('grad',grad.device)
    grad = grad*reverse_mask
    grad_y, grad_i = torch.sort(grad.flatten().abs(),descending=True)
    grad_thre = grad_y[reactivate_num]
    grad_mask = torch.where(grad.abs()>grad_thre, ones, zeros)
    mask = mask + grad_mask
    return mask
##### random pruning #######
