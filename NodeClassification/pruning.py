...
##### pruning remain mask percent #######
def get_final_mask_epoch02(model,adj_grad,wei_01_grad,wei_02_grad, adj_percent, wei_percent):
    if isinstance(adj_grad,list):
        adj_grad = torch.stack(adj_grad,dim=0)
        adj_grad = torch.mean(adj_grad,dim=0)

        wei_01_grad = torch.stack(wei_01_grad,dim=0)
        wei_01_grad = torch.mean(wei_01_grad,dim=0)

        wei_02_grad = torch.stack(wei_02_grad,dim=0)
        wei_02_grad = torch.mean(wei_02_grad,dim=0)
    ...
    # ori_adj_mask.add_((2 * torch.rand(ori_adj_mask.shape) - 1) * 1e-5)
    #mask_dict['adj_mask'] = get_each_mask(ori_adj_mask, adj_thre)
    mask_dict['adj_mask'] = get_each_mask02(ori_adj_mask, adj_thre,adj_grad,percent=adj_percent,adj=True)
    mask_dict['weight1_mask'] = get_each_mask02(model.net_layer[0].state_dict()['weight_mask_train'], wei_thre,wei_01_grad, percent=wei_percent,adj=False)
    mask_dict['weight2_mask'] = get_each_mask02(model.net_layer[1].state_dict()['weight_mask_train'], wei_thre,wei_02_grad, percent=wei_percent,adj=False)
...
def get_each_mask02(mask_weight_tensor, threshold,grad,percent,adj=False):
    ...
    reactivate_num = int(0.005*total_non_zero)#0.005
