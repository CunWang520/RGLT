...
def get_final_mask_epoch(model, rewind_weight, args):
    ...
    rewind_weight['ginlayers.0.apply_func.mlp.linear.weight_mask_train'] = get_each_mask(
        model.ginlayers[0].apply_func.mlp.linear.state_dict()['weight_mask_train'], wei_thre)
    rewind_weight['ginlayers.0.apply_func.mlp.linear.weight_mask_fixed'] = rewind_weight[
        'ginlayers.0.apply_func.mlp.linear.weight_mask_train']

    rewind_weight['ginlayers.1.apply_func.mlp.linear.weight_mask_train'] = get_each_mask(
        model.ginlayers[1].apply_func.mlp.linear.state_dict()['weight_mask_train'], wei_thre)
    rewind_weight['ginlayers.1.apply_func.mlp.linear.weight_mask_fixed'] = rewind_weight[
        'ginlayers.1.apply_func.mlp.linear.weight_mask_train']

    adj_spar = rewind_weight['adj_mask2_fixed'].sum() * 100 / model.edge_num
    wei_nonzero = rewind_weight['ginlayers.0.apply_func.mlp.linear.weight_mask_fixed'].sum() + rewind_weight[
        'ginlayers.1.apply_func.mlp.linear.weight_mask_fixed'].sum()
    wei_all = rewind_weight['ginlayers.0.apply_func.mlp.linear.weight_mask_fixed'].numel() + rewind_weight[
        'ginlayers.1.apply_func.mlp.linear.weight_mask_fixed'].numel()
    wei_spar = wei_nonzero * 100 / wei_all

    return rewind_weight, adj_spar, wei_spar


def get_final_mask_epoch02(model,adj_grad,wei_01_grad,wei_02_grad, rewind_weight, args):
    adj_percent = args['pruning_percent_adj']
    wei_percent = args['pruning_percent_wei']
    ...
    rewind_weight['adj_mask1_train'] = get_each_mask02(ori_adj_mask, adj_thre,adj_grad,adj=True)
    rewind_weight['adj_mask2_fixed'] = rewind_weight['adj_mask1_train']

    rewind_weight['ginlayers.0.apply_func.mlp.linear.weight_mask_train'] = get_each_mask02(
        model.ginlayers[0].apply_func.mlp.linear.state_dict()['weight_mask_train'], wei_thre,wei_01_grad)
    rewind_weight['ginlayers.0.apply_func.mlp.linear.weight_mask_fixed'] = rewind_weight[
        'ginlayers.0.apply_func.mlp.linear.weight_mask_train']

    rewind_weight['ginlayers.1.apply_func.mlp.linear.weight_mask_train'] = get_each_mask02(
        model.ginlayers[1].apply_func.mlp.linear.state_dict()['weight_mask_train'], wei_thre,wei_02_grad)
    rewind_weight['ginlayers.1.apply_func.mlp.linear.weight_mask_fixed'] = rewind_weight[
        'ginlayers.1.apply_func.mlp.linear.weight_mask_train']
...
def get_each_mask02(mask_weight_tensor, threshold,grad,adj=False):
    ones = torch.ones_like(mask_weight_tensor)
    zeros = torch.zeros_like(mask_weight_tensor)
    mask = torch.where(mask_weight_tensor.abs() > threshold, ones, zeros)
    total_zero = mask[mask!=1].shape[0]
    print('total_zero:',total_zero)
    reactivate_num = int(0.005*total_zero)
