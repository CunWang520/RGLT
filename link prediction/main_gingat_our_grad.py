...
def run_fix_mask(args, imp_num, rewind_weight_mask, dataset_dict):
    ...
    for epoch in range(1, args.fix_epoch + 1):
        ...
        print(
            "IMP[{}] ({} {} FIX Mask) Epoch:[{}/{}], Loss:[{:.4f}] Val:[{:.2f}] Test:[{:.2f}] | Best Val:[{:.2f}] Test:[{:.2f}] at Epoch:[{}] | Adj:[{:.2f}%] Wei:[{:.2f}%]"
            .format(imp_num,
                    args.net,
                    args.dataset,
                    epoch,
    ...
    print(
        "syd final: IMP[{}] ({} {} FIX Mask) Best Val:[{:.2f}] Test:[{:.2f}] at Epoch:[{}] | Adj:[{:.2f}%] Wei:[{:.2f}%]"
        .format(imp_num,
                args.net,
                args.dataset,
                best_val_acc['val_acc'] * 100,
...
def run_get_mask(args, imp_num, rewind_weight_mask, dataset_dict):
    ...
    if args.net == 'gin':
        model = GIC_GIN(nb_nodes, ft_size, hid_units, nonlinearity, num_clusters, 100, g)
        pruning_gin.add_mask(model.gcn)
        adj_grad = torch.zeros_like(model.gcn.adj_mask1_train).cpu()
        wei_01_grad = torch.zeros_like(model.gcn.ginlayers[0].apply_func.mlp.linear.weight_mask_train).cpu()
        wei_02_grad = torch.zeros_like(model.gcn.ginlayers[1].apply_func.mlp.linear.weight_mask_train).cpu()
    elif args.net == 'gat':
        model = GIC_GAT(nb_nodes, ft_size, hid_units, nonlinearity, num_clusters, 100, g)
        pruning_gat.add_mask(model.gcn)
        g.add_edges(list(range(nb_nodes)), list(range(nb_nodes)))
    ...
    for epoch in range(1, args.mask_epoch + 1):
        ...
        optimiser.step()
        adj_grad = adj_grad + model.gcn.adj_mask1_train.grad.clone().cpu()/args.mask_epoch
        wei_01_grad = wei_01_grad + model.gcn.ginlayers[0].apply_func.mlp.linear.weight_mask_train.grad.clone().cpu()/args.mask_epoch
        wei_02_grad = wei_02_grad + model.gcn.ginlayers[1].apply_func.mlp.linear.weight_mask_train.grad.clone().cpu()/args.mask_epoch
        with torch.no_grad():
            acc_val, _ = pruning.test(model, features,
                                      g,
                                      sparse,
            ...
            if acc_val > best_val_acc['val_acc']:
                ...
                if args.net == 'gin':
                    rewind_weight, adj_spar, wei_spar = pruning_gin.get_final_mask_epoch02(model, rewind_weight, args,adj_grad,wei_01_grad, wei_02_grad)
                else:
                    rewind_weight, adj_spar, wei_spar = pruning_gat.get_final_mask_epoch(model, rewind_weight, args)

        print(
            "IMP[{}] ({}, {} Get Mask) Epoch:[{}/{}], Loss:[{:.4f}] Val:[{:.2f}] Test:[{:.2f}] | Best Val:[{:.2f}] Test:[{:.2f}] at Epoch:[{}] | Adj:[{:.2f}%] Wei:[{:.2f}%]"
            .format(imp_num,
                    args.net,
                    args.dataset,
                    epoch,
...
def parser_loader():
    ...
    parser.add_argument('--exp_name', type=str, default='gin_our', help='')
